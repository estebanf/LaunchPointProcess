...
-- Generated 12/3/17 6:54 PM
-- Descriptor: Metrics/metrics.xvar (currently edited)
-- External variable IQQueue
DROP TABLE IF EXISTS IQQueue;
CREATE TABLE IQQueue (CASEID INTEGER, ENQUEUEDAT TIMESTAMP, CLIENTID INTEGER, ENQUEUINGPID INTEGER);
CREATE INDEX IQQueue_KEYS ON IQQueue (CASEID);
-- External variable ExecutionError
DROP TABLE IF EXISTS ExecutionError;
CREATE TABLE ExecutionError (PID VARCHAR(250), CASEID INTEGER, FAILCOUNT INTEGER, STEP VARCHAR(250), RESOLVED BOOLEAN, INTEGRATION BOOLEAN, VALIDATION BOOLEAN, MESSAGE BOOLEAN, CREATED_AT TIMESTAMP, UPDATED_AT TIMESTAMP, CREATED_BY_PROCESS VARCHAR(250), CREATED_BY_INSTANCE INTEGER);
CREATE INDEX ExecutionError_KEYS ON ExecutionError (PID);
...
CREATE TABLE IQTracking (ID INTEGER, CLIENTID INTEGER, CASEID INTEGER, IQRECEIVED BOOLEAN, IQRECEIVEDAT TIMESTAMP, IQENQUEUED BOOLEAN, IQENQUEUEDAT TIMESTAMP, IQDEQUEUED BOOLEAN, IQDEQUEUEDAT TIMESTAMP, IQSENT BOOLEAN, IQSENTAT TIMESTAMP, CREATED_AT TIMESTAMP, UPDATED_AT TIMESTAMP, CREATED_BY_PROCESS VARCHAR(250), CREATED_BY_INSTANCE INTEGER);
