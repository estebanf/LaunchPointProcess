...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"	version="2.0"	xmlns:getCaseActivities-process="http://bpms.everteam.com/Processes/Integrations/getCaseActivities/process"	xmlns:getCaseDiaries-process="http://bpms.everteam.com/Processes/Integrations/getCaseDiaries/process"	xmlns:Launchpoint="http://www.example.org/Launchpoint"	xmlns:Database="http://bpms.everteam.com/Processes/Core/CaseEvaluator/Database"	xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:diag="http://bpms.everteam.com/Processes/Core/CaseEvaluator"	xmlns:client="http://bpms.everteam.com/Processes/Core/CaseEvaluator/client"	xmlns:this="http://bpms.everteam.com/Processes/Core/CaseEvaluator/process"	xmlns:tns="http://everteam.com/xvar/example" xmlns:DCM="http://bpms.everteam.com/Processes/Core/CaseEvaluator/DCM"	xmlns:getDCMCase-process="http://bpms.everteam.com/Processes/Integrations/getDCMCase/process"	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"	xmlns:getCaseDetails-process="http://bpms.everteam.com/Processes/Integrations/getCaseDetails/process">
  ...
  <xsl:template match="/Launchpoint:CaseActivities">
    ...
    <xsl:variable name="iqresponse" select="count(Launchpoint:activities[Launchpoint:ActivityType=('IQ Incoming Phone Call: IQ Response','IQ received via mail','IQ received via phone','IQ received via web','IQ received via fax','IQ Received Via Mail','IQ Received via Mail, Missing Information')]) > 0">
    </xsl:variable>
		<xsl:variable name="iqresponseReason">
			<xsl:choose>
				<xsl:when test="$iqresponse">
					<xsl:for-each select="Launchpoint:activities[Launchpoint:ActivityType=('IQ Incoming Phone Call: IQ Response','IQ received via mail','IQ received via phone','IQ received via web','IQ received via fax','IQ Received Via Mail','IQ Received via Mail, Missing Information')]">
						<xsl:sort select="Launchpoint:CreatedDate" order="descending" />
						<xsl:if test="position() = 1">
							<xsl:value-of select="Launchpoint:ActivityType"/>
						</xsl:if>
					</xsl:for-each>
				</xsl:when>
    ...
    <xsl:variable name="isoSent" select="count(Launchpoint:activities[Launchpoint:ActivityType=('ISO Index Received: Pending')]) > 0">
    </xsl:variable>
    <xsl:variable name="iqLetters">
      <xsl:choose>
        <xsl:when test="$iqreturnmail">
          <xsl:value-of select="count(Launchpoint:activities[Launchpoint:ActivityType=('Outgoing Letter : IQ(FIRST)','Outgoing Letter : IQ(SECOND)','Outgoing Letter : IQ(THIRD)','Outgoing Letter : IQ(FOURTH)') and xs:dateTime(Launchpoint:CreatedDate) &gt; xs:dateTime($iqreturnmaildate)])" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="count(Launchpoint:activities[Launchpoint:ActivityType=('Outgoing Letter : IQ(FIRST)','Outgoing Letter : IQ(SECOND)','Outgoing Letter : IQ(THIRD)','Outgoing Letter : IQ(FOURTH)')])" />
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    ...
    <Launchpoint:LPCase>
      <Launchpoint:CaseId>
        <xsl:value-of select="$dcmCase/Launchpoint:CaseId" />
      </Launchpoint:CaseId>
      <Launchpoint:BatchId>
        <xsl:value-of select="$dcmCase/Launchpoint:BatchId" />
      </Launchpoint:BatchId>
      <Launchpoint:ClientId>
        <xsl:value-of select="$dcmCase/Launchpoint:ClientId" />
      </Launchpoint:ClientId>
      <Launchpoint:EnvironmentId>
        <xsl:value-of select="$dcmCase/Launchpoint:EnvironmentId" />
      </Launchpoint:EnvironmentId>
      <Launchpoint:ISOIndicator>
        <xsl:value-of select="$dcmCase/Launchpoint:ISOIndicator" />
      </Launchpoint:ISOIndicator>
      <Launchpoint:Score>
        <xsl:value-of select="$dcmCase/Launchpoint:Score" />
      </Launchpoint:Score>
      <Launchpoint:AccidentDate>
        <xsl:value-of select="$dcmCase/Launchpoint:AccidentDate" />
      </Launchpoint:AccidentDate>
      <Launchpoint:BenefitAmount>
        <xsl:value-of select="$dcmCase/Launchpoint:BenefitAmount" />
      </Launchpoint:BenefitAmount>
      <Launchpoint:FundingSource>
        <xsl:value-of select="$dcmCase/Launchpoint:FundingSource" />
      </Launchpoint:FundingSource>
      <Launchpoint:LOB>
        <xsl:value-of select="$dcmCase/Launchpoint:LOB" />
      </Launchpoint:LOB>
      <Launchpoint:WorkComp>
        <xsl:value-of select="$dcmCase/Launchpoint:WorkComp" />
      </Launchpoint:WorkComp>
      <Launchpoint:CaseStatus>
        <xsl:value-of select="$dcmCase/Launchpoint:CaseStatus" />
      </Launchpoint:CaseStatus>
      <Launchpoint:CaseSource>
        <xsl:value-of select="$dcmCase/Launchpoint:CaseSource" />
      </Launchpoint:CaseSource>
      <Launchpoint:LifeCycle>
        <xsl:value-of select="$dcmCase/Launchpoint:LifeCycle" />
      </Launchpoint:LifeCycle>
      <Launchpoint:CloseReason>
        <xsl:value-of select="$dcmCase/Launchpoint:CloseReason" />
      </Launchpoint:CloseReason>
      <Launchpoint:State>
        <xsl:value-of select="$dcmCase/Launchpoint:State" />
      </Launchpoint:State>
      <Launchpoint:IQResponseReceived>
        <xsl:value-of select="$iqresponse" />
      </Launchpoint:IQResponseReceived>
      <Launchpoint:IQResponseReceivedReason>
        <xsl:value-of select="$iqresponseReason" />
      </Launchpoint:IQResponseReceivedReason>
      <Launchpoint:CanClose>
        <xsl:choose>
          <xsl:when test="xs:boolean($dcmCase/Launchpoint:CanClose)">
            <xsl:value-of select="$canClose" />
          </xsl:when>
          <xsl:otherwise>false</xsl:otherwise>
        </xsl:choose>
      </Launchpoint:CanClose>
      <Launchpoint:BelowCostEffectivePursuit>
        <xsl:value-of select="$dcmCase/Launchpoint:BelowCostEffectivePursuit" />
      </Launchpoint:BelowCostEffectivePursuit>
      <Launchpoint:IQReturnMail>
        <xsl:value-of select="$iqreturnmail" />
      </Launchpoint:IQReturnMail>
      <Launchpoint:InvalidAddress>
        <xsl:value-of select="$dcmCase/Launchpoint:InvalidAddress" />
      </Launchpoint:InvalidAddress>
      <Launchpoint:IQHold>
        <xsl:value-of select="$iqHold" />
      </Launchpoint:IQHold>
      <Launchpoint:ISOSent>
        <xsl:value-of select="$isoSent" />
      </Launchpoint:ISOSent>
      <Launchpoint:ISOResponse>
        <xsl:value-of select="$dcmCase/Launchpoint:ISOResponse" />
      </Launchpoint:ISOResponse>
      <Launchpoint:vip>false</Launchpoint:vip>
      <Launchpoint:IQLetters>
        <xsl:value-of select="$iqLetters" />
      </Launchpoint:IQLetters>
      <Launchpoint:daysSinceAccident>
        <xsl:value-of select="$dcmCase/Launchpoint:daysSinceAccident" />
      </Launchpoint:daysSinceAccident>
      <Launchpoint:BelowCostEffectivePursuitDueDatePassed>
        <xsl:value-of select="$dcmCase/Launchpoint:BelowCostEffectivePursuitDueDatePassed" />
      </Launchpoint:BelowCostEffectivePursuitDueDatePassed>
      <Launchpoint:daysSinceLastIQLetter>
        <xsl:value-of select="$daysSinceLastIQLetter" />
      </Launchpoint:daysSinceLastIQLetter>
    </Launchpoint:LPCase>
  </xsl:template>
</xsl:stylesheet>
