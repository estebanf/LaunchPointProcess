...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	version="2.0"
	xmlns:getLaunchpointCase="http://bpms.everteam.com/Services/REST/getLaunchpointCase.rest"
	xmlns:Launchpoint="http://www.example.org/Launchpoint" 
	xmlns:xs="http://www.w3.org/2001/XMLSchema"
	xmlns:diag="http://bpms.everteam.com/Processes/Integrations/getDCMCase"
	xmlns:getLaunchpointCase1="__project:///Schemas/integrations/getLaunchpointCase.xsd"
	xmlns:client="http://bpms.everteam.com/Processes/Integrations/getDCMCase/client"
	xmlns:processimplicitPartner="http://bpms.everteam.com/Processes/Integrations/getDCMCase/processimplicitPartner"
	xmlns:this="http://bpms.everteam.com/Processes/Integrations/getDCMCase/process"
	...
	xmlns:Validation="http://www.example.org/Validation">
	<xsl:output />
...
<!-- 	EnvironmentId -->
		<xsl:variable name="chkEnvironmentId">
			<xsl:choose>
				<xsl:when test="EnvironmentId">
					<xsl:call-template name="validator">
						<xsl:with-param name="obj" select="EnvironmentId" />
						<xsl:with-param name="dataType" select="'string'" />
					</xsl:call-template>			
				</xsl:when>
				<xsl:otherwise>false</xsl:otherwise>
		...
		<Validation:DocumentValid>
			<xsl:choose>
				...
				<xsl:when test="$chkEnvironmentId = 'false'">
		 			<Validation:isValid>false</Validation:isValid>
					<Validation:element>EnvironmentId</Validation:element>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template name="validator">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="$dataType = 'integer' "><xsl:value-of select="string($obj/text()) castable as xs:integer" /></xsl:when>
					<xsl:when test="$dataType = 'string' "><xsl:value-of select="string($obj/text()) castable as xs:string" /></xsl:when>
					<xsl:when test="$dataType = 'double' "><xsl:value-of select="string($obj/text()) castable as xs:double" /></xsl:when>
					<xsl:when test="$dataType = 'boolean' "><xsl:value-of select="string($obj/text()) castable as xs:boolean" /></xsl:when>
					<xsl:when test="$dataType = 'date' "><xsl:value-of select="string($obj/text()) castable as xs:date" /></xsl:when>
					<xsl:when test="$dataType = 'datetime' "><xsl:value-of select="string($obj/text()) castable as xs:dateTime" /></xsl:when>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
